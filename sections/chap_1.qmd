...
4.  Equation (@eq-contradiction-root-2-2) implies 
$$
m^2 = 2 n^2 \,.
...
5. Since $m^2$ is an even number, it follows that also $m$ is an even number. Then there exists $p \in \N$ such that 
$$
m = 2p \,.
$$ {#eq-contradiction-root-2-4}

   
6. Substitute (@eq-contradiction-root-2-4) in (@eq-contradiction-root-2-3) to get
$$
...
7. We now make a series of observations:
    * Equation (@eq-contradiction-root-2-5) says that $n^2$ is even.
    * The same argument in Step 5 guarantees that also $n$ is even.
    * We have already seen that $m$ is even.
    * Therefore $n$ and $m$ are both even.  
    * Hence $n$ and $m$ have $2$ as common factor.
    * But Step 2 says that $n$ and $m$ have no common factors.
    * **Contradiction**

8. Our reasoning has run into a **contradiction**, stemming  from assumption (@eq-contradiction-root-2-false). Therefore (@eq-contradiction-root-2-false) is **FALSE**, and so 
$$
\sqrt{2} \notin \Q \,
$$
ending the proof.
